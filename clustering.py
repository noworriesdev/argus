...
def cluster_messages_from_neo4j():
    ...
    for channel_id in channel_ids:
        with driver.session() as session:
            messages = session.read_transaction(get_channel_messages_and_timestamps, channel_id)
        ...
        if not messages:
            ...
            timestamps_array = np.array(timestamps_unix).reshape(-1, 1).astype(np.float64)
            ...
            synthesized_labels = [clusters[label] for label in sorted(clusters.keys()) if label != -1]
...
def get_dominating_user_clusters(original_clusters, user_id, threshold=0.6):
    ...
    for cluster in original_clusters:
        user_word_counts = get_user_word_counts(cluster)  # get word counts for each user in cluster
        total_word_count = get_word_count(cluster)  # get total word count for the cluster
        
        if user_dominates_cluster(user_word_counts, user_id, total_word_count, threshold):
...
def convert_cluster_to_tuning_structure(cluster, assistant_id):
    results_dict = {"messages": [
        {"role": "system", "content": "You are Lucy Bot, an autonomous being. You are trained to emulate the behavior of Lucy, a real human. You should express yourself truthfully in all interactions."}
    ]}
    for message in cluster:
    ...
    with open('output.jsonl', 'w', encoding='utf-8') as f:
        ...
        for entry in json_to_dump:
            ...
            if not any(message["role"] == "user" for message in entry["messages"]):
                entry["messages"].append({"role": "user", "content": "what do you think, Lucy?"})
